...
import { ArrowDownRight, ArrowRight, ArrowUpRight, ArrowSquareOut, BookmarkSimple, CaretDown, ChartLineUp, Check, Circle, FunnelSimple, GameController, Info, MagnifyingGlass, MoonStars, Star, Sun, TrendUp, UsersThree, X } from "@phosphor-icons/react";
...
function Header({ view, onNavigate, search, setSearch, theme, onToggleTheme }) { return <header className={`site-header ${view === "home" ? "site-header-overlay" : ""}`}><button className="brand" onClick={() => onNavigate("home")} aria-label="SteamTwo, início"><span>Steam</span><b>Two</b></button><nav aria-label="Navegação principal"><button className={view === "home" ? "active" : ""} onClick={() => onNavigate("home")}>Início</button><button className={view === "catalog" ? "active" : ""} onClick={() => onNavigate("catalog")}>Catálogo</button><button className={view === "rankings" ? "active" : ""} onClick={() => onNavigate("rankings")}>Rankings</button></nav><label className="search-box"><MagnifyingGlass size={20} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar jogos" aria-label="Buscar jogos" /></label><button className="theme-toggle" aria-label={theme === "light" ? "Ativar tema escuro" : "Ativar tema claro"} onClick={onToggleTheme}>{theme === "light" ? <MoonStars size={19} /> : <Sun size={19} />}</button></header>; }
...
function Detail({ game, onBack, onMethodology }) { const price = bestPrice(game.prices); return <section className="detail-page"><button className="back-link" onClick={onBack}><ArrowRight size={17} /> Voltar</button><div className="detail-hero" style={{ "--detail-image": `url(${game.heroUrl})` }}><div><span className="eyebrow">DETALHE DO JOGO</span><h1>{game.shortTitle}</h1><div className="detail-tags">{game.genres.map((genre) => <span key={genre}>{genre}</span>)}</div><p>{game.summary}</p>{price && <div className="detail-price">{price.isFree ? <span className="promo-badge promo-free-tag">Grátis</span> : price.discountPercent > 0 ? <><span className="promo-badge">-{price.discountPercent}%</span><del>{formatCurrency(price.initialAmount, price.currency)}</del><ins>{formatCurrency(price.finalAmount, price.currency)}</ins></> : <ins>{formatCurrency(price.finalAmount, price.currency)}</ins>}</div>}<div className="detail-actions"><a className="primary-button" href={storeUrlForGame(game)} target="_blank" rel="noreferrer">Comprar <ArrowSquareOut size={17} /></a><button className="list-button"><BookmarkSimple size={19} /> Adicionar à lista</button></div></div><div className="detail-score"><span>ÍNDICE STEAMTWO</span><Score value={game.score} large /><small>/ 100</small><button onClick={onMethodology}>Como calculamos <Info size={14} /></button></div></div><div className="detail-columns"><article><h2>Sobre este jogo</h2><p>{game.summary} O índice combina tração, qualidade e engajamento para oferecer uma leitura comparável entre os títulos.</p></article><aside><h2>Fontes</h2><div className="source-pills">{game.stores.map((store) => <span key={store}><Check size={14} /> {storeLabel(store)}</span>)}<span><Check size={14} /> IGDB</span></div><small>Última atualização: 24 de ago. de 2026</small></aside></div><PopularityChart slug={game.slug} /></section>; }
